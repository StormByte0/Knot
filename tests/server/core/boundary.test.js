...
function isDocumentationContent(line) {
        ...
        describe(`${dir}/ — no imports from formats/`, () => {
            it('should not import from formats/', () => {
                ...
                if (violations.length > 0) {
                    assert.fail(`Import from formats/ found in ${dir}/!\n` +
                        violations.join('\n') +
            ...
            it('should contain no format name references in source code', () => {
                ...
                for (const file of files) {
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        ...
                        if (isCommentLine(line))
                            continue;
                        if (isDocumentationContent(line))
                            continue;
                        for (const { pattern, name } of forbiddenFormatPatterns) {
                            if (pattern.test(line)) {
                                violations.push(`${path.relative(process.cwd(), file)}:${i + 1}: [${name}] ${line.trim()}`);
                            }
                        }
                    }
                }
                if (violations.length > 0) {
                    assert.fail(`Format name references found in ${dir}/!\n` +
                        violations.join('\n') +
            ...
            it('should not contain hardcoded <<>> macro patterns in source code', () => {
                ...
                for (const file of files) {
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        ...
                        if (isCommentLine(line))
                            continue;
                        if (line.includes('MUST NOT') || line.includes('close-tag') || line.includes('CloseTag'))
                            continue;
                        if (line.includes("'close-tag'") || line.includes('"close-tag"'))
                            continue;
                        // Allow MacroBodyStyle enum references
                        if (line.includes('MacroBodyStyle'))
                            continue;
                        // Allow ISyntaxProvider references (documentation about the interface)
                        if (line.includes('ISyntaxProvider'))
                            continue;
                ...
                if (violations.length > 0) {
                    assert.fail(`Hardcoded <<>> pattern found in ${dir}/!\n` +
                        violations.join('\n') +
        ...
        it('should not hardcode $ = story or _ = temp variable logic', () => {
            ...
            for (const file of files) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (line.includes('classifyVariableSigil'))
                        continue;
            ...
            if (violations.length > 0) {
                assert.fail(`Hardcoded sigil logic found in core/!\n` +
                    violations.join('\n') +
...
//# sourceMappingURL=boundary.test.js.map