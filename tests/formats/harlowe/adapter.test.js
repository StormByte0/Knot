...
// Import the adapter
const adapter_1 = require("../../../server/src/formats/harlowe/adapter");
const hookTypes_1 = require("../../../server/src/hooks/hookTypes");
describe('HarloweAdapter', () => {
    let adapter;
    before(() => {
        adapter = new adapter_1.HarloweAdapter();
    });
    // ─── IFormatProvider ──────────────────────────────────────────
    describe('IFormatProvider', () => {
        ...
        it('should support all expected capabilities', () => {
            assert.ok(adapter.capabilities.has('macroCompletion'), 'Missing macroCompletion');
            assert.ok(adapter.capabilities.has('passageCompletion'), 'Missing passageCompletion');
            assert.ok(adapter.capabilities.has('variableCompletion'), 'Missing variableCompletion');
            assert.ok(adapter.capabilities.has('macroHover'), 'Missing macroHover');
            assert.ok(adapter.capabilities.has('customDiagnostics'), 'Missing customDiagnostics');
            assert.ok(adapter.capabilities.has('bodyLexing'), 'Missing bodyLexing');
        });
    ...
    describe('IMacroProvider', () => {
        ...
        it('should classify changers correctly', () => {
            ...
            for (const name of changers) {
                ...
                assert.strictEqual(macro.kind, hookTypes_1.MacroKind.Changer, `${name} should be a Changer, got ${macro.kind}`);
            }
        });
        it('should classify commands correctly', () => {
            const commands = ['go-to:', 'print:', 'link-goto:', 'link-undo:', 'show:', 'hide:'];
            for (const name of commands) {
                const macro = macroProvider().getMacroByName(name);
                assert.ok(macro, `Macro ${name} not found`);
                assert.strictEqual(macro.kind, hookTypes_1.MacroKind.Command, `${name} should be a Command, got ${macro.kind}`);
            }
        });
        it('should classify instants correctly', () => {
            ...
            for (const name of instants) {
                const macro = macroProvider().getMacroByName(name);
                assert.ok(macro, `Macro ${name} not found`);
                assert.strictEqual(macro.kind, hookTypes_1.MacroKind.Instant, `${name} should be an Instant, got ${macro.kind}`);
            }
        });
    ...
    describe('IPassageProvider', () => {
        ...
        it('should classify header passages as Special', () => {
            ...
            assert.strictEqual(kind, hookTypes_1.PassageKind.Special);
        });
        ...
        it('should include Harlowe passage types', () => {
            ...
            assert.ok(types.has(hookTypes_1.PassageType.Header), 'Missing Header passage type');
            assert.ok(types.has(hookTypes_1.PassageType.Footer), 'Missing Footer passage type');
            assert.ok(types.has(hookTypes_1.PassageType.Startup), 'Missing Startup passage type');
        });
    ...
    describe('ILinkProvider', () => {
        ...
        it('should parse simple [[Target]] links', () => {
            ...
            assert.strictEqual(link.kind, hookTypes_1.LinkKind.Passage);
        });
        ...
        it('should detect external URLs', () => {
            ...
            assert.ok(link);
            assert.strictEqual(link.kind, hookTypes_1.LinkKind.External);
        });
        ...
        it('should support Passage and External link kinds', () => {
            ...
            assert.ok(kinds.includes(hookTypes_1.LinkKind.Passage), 'Missing Passage link kind');
            assert.ok(kinds.includes(hookTypes_1.LinkKind.External), 'Missing External link kind');
        });
    });
    // ─── ISyntaxProvider ──────────────────────────────────────────
    describe('ISyntaxProvider', () => {
        ...
        it('should use Hook macro body style', () => {
            assert.strictEqual(syntaxProvider().getMacroBodyStyle(), hookTypes_1.MacroBodyStyle.Hook);
        });
    ...
    describe('IDiagnosticProvider', () => {
        ...
        it('should support Harlowe-specific diagnostic rules', () => {
            ...
            assert.ok(rules.includes(hookTypes_1.DiagnosticRule.UnknownMacro), 'Missing UnknownMacro rule');
            assert.ok(rules.includes(hookTypes_1.DiagnosticRule.InvalidHookStructure), 'Missing InvalidHookStructure rule');
            assert.ok(rules.includes(hookTypes_1.DiagnosticRule.InvalidChangerBinding), 'Missing InvalidChangerBinding rule');
        });
    });
});
