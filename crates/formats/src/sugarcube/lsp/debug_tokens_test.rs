...
fn debug_tokens_for_expression() {
    ...
    let samples: &[(&str, &str)] = &[
        ...
        (
            "verbatim",
            "Before \"\"\"<<set $x to 1>> //italic//\"\"\" After",
        ),
        ...
        (
            "tb-verbatim",
            "Before \"\"\"This //won't// be formatted as italics.\"\"\" After",
        ),
        // H1: nowiki (similar to verbatim)
        (
            "tb-nowiki",
            "Before <nowiki>Same here — __no__ formatting.</nowiki> After",
        ),
    ];
...
fn debug_js_analysis_for_expression() {
    ...
    for node in &ast.nodes {
        if let crate::sugarcube::ast::AstNode::Expression {
            content,
            js_analysis,
            ..
        } = node
        {
            ...
            for ms in &analysis.js_method_spans {
                let snip = if ms.start < src.len() && ms.end <= src.len() {
                    &src[ms.start..ms.end]
                } else {
                    "<OOB>"
                };
...
fn debug_js_analysis_for_property_fn() {
    ...
    annotate_js(&mut ast, src, false, true, &HashSet::new());
    for node in &ast.nodes {
        if let crate::sugarcube::ast::AstNode::Macro {
            name,
            args,
            js_analysis,
            ..
        } = node
        {
            ...
            println!("function_calls ({}):", analysis.function_calls.len());
            for fc in &analysis.function_calls {
                let snip = if fc.span.start < src.len() && fc.span.end <= src.len() {
                    &src[fc.span.start..fc.span.end]
                } else {
                    "<OOB>"
                };
            ...
            for ps in &analysis.js_property_spans {
                let snip = if ps.start < src.len() && ps.end <= src.len() {
                    &src[ps.start..ps.end]
                } else {
                    "<OOB>"
                };
