...
static RE_MACRO_ADD: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"Macro\.add\s*\(\s*['"]([A-Za-z_][A-Za-z0-9_]*)['"]\s*,\s*\{[\s\S]*?handler\s*:\s*function\s*\(([^)]*)\)\s*\{([\s\S]*?)\}"#)
    .unwrap()
});

/// `Macro.add(['name1', 'name2'], { ... })` — multiple macro names
static RE_MACRO_ADD_MULTI: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"Macro\.add\s*\(\s*\[([^\]]*)\]\s*,\s*\{[\s\S]*?handler\s*:\s*function\s*\(([^)]*)\)\s*\{([\s\S]*?)\}"#)
    .unwrap()
});
    ...
    for passage in passages {
        ...
        if passage.tags.contains(&"script".to_string()) {
            ...
            for caps in RE_MACRO_ADD.captures_iter(&stripped) {
                ...
                let handler_params = caps.get(2).unwrap().as_str().to_string();
                let handler_body = caps.get(3).unwrap().as_str().to_string();
