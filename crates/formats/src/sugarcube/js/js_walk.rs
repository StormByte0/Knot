...
fn walk_expression(
    ...
    match expr {
        ...
        Expr::ArrayExpression(arr) => {
            ...
            //
            // ArrayExpressionElement inherits Expression variants via oxc's
            // @inherit Expression macro. We handle SpreadElement and Elision
            // explicitly, then match on the inherited Expression variants.
            // For nested ObjectExpression/ArrayExpression, we handle them
            // inline (same logic as the handlers above/below) since oxc AST
            // types are arena-allocated and can't be converted to &Expression
            // via the From impl (which takes ownership).
            ...
            for elem in &arr.elements {
                match elem {
                    ...
                    // Nested object literal inside array: [{a:1}, {b:2}]
                    ...
                    // Nested array inside array: [[1,2], [3,4]]
                    ArrayExpressionElement::ArrayExpression(inner_arr) => {
                        for inner_elem in &inner_arr.elements {
                            // Recurse using the same match logic by calling
                            // walk_expression on a synthetic expression.
                            // Since we can't construct Expression from &ArrayExpressionElement,
                            // we just re-run the array handler logic inline.
                            // This handles arbitrary nesting depth.
                            if let ArrayExpressionElement::SpreadElement(spread) = inner_elem {
                                walk_expression(&spread.argument, preprocessed, analysis);
                            } else if let ArrayExpressionElement::NumericLiteral(n) = inner_elem {
                                let span = preprocessed.map_range_to_original(
                                    n.span.start as usize..n.span.end as usize
                                );
                                analysis.literal_spans.push(LiteralSpan {
                            ...
                            } else if let ArrayExpressionElement::StringLiteral(s) = inner_elem {
                                let span = preprocessed.map_range_to_original(
                                    s.span.start as usize..s.span.end as usize
                                );
                                analysis.literal_spans.push(LiteralSpan {
                            ...
                            } else if let ArrayExpressionElement::BooleanLiteral(b) = inner_elem {
                                let span = preprocessed.map_range_to_original(
                                    b.span.start as usize..b.span.end as usize
                                );
                                analysis.literal_spans.push(LiteralSpan {
                            ...
                            } else if let ArrayExpressionElement::NullLiteral(n) = inner_elem {
                                let span = preprocessed.map_range_to_original(
                                    n.span.start as usize..n.span.end as usize
                                );
                                analysis.literal_spans.push(LiteralSpan {
                            ...
                            } else if let ArrayExpressionElement::Identifier(id) = inner_elem {
                                check_identifier_for_substituted_var(id, false, preprocessed, analysis);
                            } else if let ArrayExpressionElement::ObjectExpression(obj) = inner_elem {
                                for prop in &obj.properties {
                                    if let oxc_ast::ast::ObjectPropertyKind::ObjectProperty(p) = prop {
                                        walk_expression(&p.value, preprocessed, analysis);
                                    }
                            ...
                            // Other nested types (calls, binary, etc.) inside
                            // nested arrays are not handled here — the common
                            // cases (literals, identifiers, objects, arrays)
                            // are covered above.
                        }
                    }
                    // Binary/logical expressions inside arrays: [1+2, $x || $y]
                    ArrayExpressionElement::BinaryExpression(bin) => {
                        walk_expression(&bin.left, preprocessed, analysis);
                        walk_expression(&bin.right, preprocessed, analysis);
                    }
                    ArrayExpressionElement::LogicalExpression(logic) => {
                        walk_expression(&logic.left, preprocessed, analysis);
                        walk_expression(&logic.right, preprocessed, analysis);
                    }
                    // Call expressions inside arrays: [myFunc(), $arr.push(1)]
                    ...
                    // Member expressions inside arrays: [$obj.prop, $arr[0]]
                    ArrayExpressionElement::StaticMemberExpression(m) => {
                        walk_expression(&m.object, preprocessed, analysis);
                    }
                    ArrayExpressionElement::ComputedMemberExpression(m) => {
                        walk_expression(&m.object, preprocessed, analysis);
                        walk_expression(&m.expression, preprocessed, analysis);
                    }
                    // Parenthesized expressions: [(1 + 2), ($x)]
                    ArrayExpressionElement::ParenthesizedExpression(pe) => {
                        walk_expression(&pe.expression, preprocessed, analysis);
                    }
                    // Other rare variants — skip for now
                    _ => {}
                }
            }
        }
