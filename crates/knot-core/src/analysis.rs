...
impl AnalysisEngine {
    ...
    fn detect_dead_end_passages(workspace: &Workspace) -> Vec<GraphDiagnostic> {
        ...
        for doc in workspace.documents() {
            for passage in &doc.passages {
                ...
                let has_graph_edges = workspace.graph.outgoing_neighbors(&passage.name).len() > 0;
    ...
    fn detect_missing_start_link(
        ...
        let has_outgoing = !start_p.links.is_empty()
            || workspace.graph.outgoing_neighbors(start_passage).len() > 0;
    ...
    pub fn collect_special_passage_initializers(
        ...
        for (_name, data) in passage_data {
    ...
    fn run_dataflow(
        ...
        if let Some(data) = passage_data.get(start_passage) {
            if let Some(state) = flow_states.get_mut(start_passage) {
