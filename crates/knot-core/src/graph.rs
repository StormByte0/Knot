...
impl PassageGraph {
        ...
        for scc in &sccs {
            ...
            if scc.len() < 2 {
                ...
                if let Some(&idx) = scc.first() {
                    ...
                    {
                        let node = &self.graph[idx];
                        // Check if there are variable writes in this passage
                        let has_mutation = passage_vars
                            .get(&node.name)
                            .map(|ops| ops.iter().any(|v| v.kind == VarKind::Write))
                            .unwrap_or(false);
            ...
            // Multi-node cycle: check if any passage in the cycle has variable writes
            let _scc_names: HashSet<String> = scc
                .iter()
    ...
    pub fn export_graph_with_metadata(
        ...
        _unreachable: &std::collections::HashSet<String>,
    ) -> GraphExport {
