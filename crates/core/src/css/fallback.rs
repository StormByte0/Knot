//! CSS fallback tokenizer (plan.md Phase 6 — demoted from the Phase 4.2
//! primary).
//!
//! A hand-rolled, highlighter-grade tokenizer: never panics, never fails,
//! tolerant of malformed input. Since Phase 6 it serves exactly two roles:
//!
//! 1. **Degenerate path for [`super::parser::parse_css`]** — when the real
//!    parser (`oxc-css-parser`) fails to produce a stylesheet at all (a hard
//!    `Err`, which its statement-level recovery makes rare), this scanner
//!    keeps highlighting alive so tokens never go dark.
//! 2. **`style="…"` attribute values** — [`parse_css_declarations`] seeds
...
/// Phase 4.2). Equivalent to [`tokenize_css_fallback`] but seeded as if
/// already inside a declaration block, so `color: red` classifies `color`
/// as a Property.
...
mod tests {
    ...
    fn unclosed_block_is_reported_once() {
        let outcome = tokenize_css_fallback(".a { color: red;");
    ...
    fn never_panics_on_garbage() {
        for src in ["", "{", "}}}", "\"unterminated", "@media {", ".a { -- } }"] {
            let _ = tokenize_css_fallback(src);
        }
    }
}
